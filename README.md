...
它不是摘要工具，也不是简单的“模仿某某风格”提示词，而是让 agent 把一批完整文章拆解成可复用的 Writing DNA：语言、结构、选题、素材、认知框架与视觉风格。蒸馏完成后，agent 可以在写作前重新读取这些风格文档，从而更稳定地复刻目标写作风格。
...
```text
请使用 writing-dna-skill 分析这个目录里的所有文章。

要求：
1. 先检查 raw/ 或 raw-corpus/ 中的文章是否足够完整。
2. 按 L1-L6 六个层次完成蒸馏。
3. 输出 语言DNA.md、文章结构模板.md、写作视角与认知框架.md、视觉风格指南.md 和 Writing-DNA.md。
4. 如果文章里有图片、截图或排版信息，请把视觉风格也纳入分析。
```
...
1. 先布置写作任务：主题、读者、篇幅、平台、要达成的效果。
2. 再让 agent 重新阅读全部蒸馏产物。
3. 要求 agent 按 `Writing-DNA.md` 中的语言、结构、认知和视觉规则写作。
...
然后根据这些风格规则，帮我写一篇面向小红书读者的文章。主题是：……
```

## 为什么有效

这个 skill 的核心不是让 agent 记住几篇文章，而是让它把风格拆成多个可检查的层次：

- 用元数据标注理解每篇文章的类型、主题、hook 和结构
- 用脚本或统计方法提取词频、句长、标点、段落节奏
- 用结构标注提炼开头、展开、转折和结尾模板
- 用深度阅读归纳选题逻辑、素材偏好和认知框架
- 对图文内容继续分析配图、截图、排版、字体层级和色彩

这样得到的不是“像某某一样写”的一句提示词，而是一组可以被 agent 反复读取、执行和自检的风格规则。
