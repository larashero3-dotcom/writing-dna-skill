...
基于 300 篇 AI 输出（5 个模型，117.9 万汉字）与 329 篇人类文章（164.8 万汉字）的实测对比。数字只用来判断某个句式够不够格立为规则，不能据此增加新的改写标准。规则的依据是触发标记可定位，不是这些频率。

完整数据、被推翻的预设和测量脚本见研究仓库 [lieflat-less-ai-tone](https://github.com/larashero3-dotcom/lieflat-less-ai-tone)。人类语料规模有限，所有结论都应视为初步。
...
| 特征 | 人类 | AI | 差异 |
|------|------|-----|------|
| 翻案腔 | 直接判断 | 先虚立误解，再推翻 | “不是…而是…”等句式尤多 |
| 顿号罗列 | 1.78/千字 | 3.21/千字 | AI 约 1.8 倍 |
| 相邻句结构 | 随内容变化 | 连续几句同一骨架 | AI 约 2 倍，各家一致 |
| 段首零主语评论 | 0.13-0.15% | 0.61% | AI 约 4 倍 |
| 材料密度 | 数字 17.92/千字 | 6.34/千字 | 人类 2.8 倍，覆盖面最广 |
| 破折号 | 表意确有需要时使用 | 用于停顿、揭晓和补充 | DeepSeek/Claude 最重，GPT 几乎不用 |
| 提示性冒号 | 少见 | 用于“总结：”“核心是：”等提示 | AI 是人类 4 倍，DeepSeek/Claude 最重 |
| 拟人化喻体 | 0.002/千字 | 0.018/千字 | AI 约 7 倍 |
| 翻译腔 | 只有五种结构可判 | 长定语、“当…时”、话题壳、句首连接词、复述句 | 其余翻译腔不算 AI 味 |
...
| 特征 | 为什么不改 |
|------|-----------|
| 单字虚词偏少（就/很/了） | AI 确实偏少，但方向是补不是删。补虚词无法验收，且容易把正式文章改成口语 |
| 反复写全称、少用代词 | 实测相邻句重复同一名词，人类比 AI 更多 |
| 被动句 | 抽象被动（“被认为／被视为”）仅 0.09/千字，远低于收录门槛 |
| 正文里的“首先……其次” | 实测与人类写作无差别。第 6 条只管小标题编号 |
| 句内同构排比 | “提升效率，降低成本，优化体验”“更高效、更专注、更有创造力”“要…要…要…”这类实测人类用得不比 AI 少（同字开头两项人类 3.87 vs AI 2.35）。只处理句间重复，见第 3 条 |
| 比喻独立成段 | 人类 0.21%、AI 0.03%，人类更多。读着突兀多半是缺回指，按第 11 条补，不要删比喻 |
| 问句、设问、问句小标题 | 查过三轮都立不住：正文问句人类是 AI 的 17 倍；按小标题总数算，问句小标题 AI 2.7%、人类最高 3.6%，无差异 |
| 名词化、长句本身 | 现代汉语书面语的正常写法，人类同样这么用 |
| 句长/段落均匀度 | 修正切句后重测：句长 CV 人类 0.67 vs AI 0.58，相邻句长差 21.7 vs 21.7，段长稳健离散度 1.00 vs 0.94。原先的 ×51 是切句缺陷造成的假象 |
| 抽象名词配具体动词 | “时间替他保管了细节”“焦虑显出形状”这类实测 0.001/千字，AI 人类都几乎不写 |
| 比喻本身 | 人类用比喻是 AI 的 2.4 倍，独立成段用比喻是 8 倍。只处理理想化拟人喻体，见第 7 条 |

### 模型差异

300 篇五模型语料实测（每千字）。同一个特征在不同模型上的量级差很大，不要按“某个模型的味”去套判断，只按规则的触发标记逐句认。
...
破折号原先记作“Claude 独有”，实测 DeepSeek 更重，GPT 几乎不用。提示性冒号各家都有，DeepSeek 0.43、Claude 0.38 最重，且人类只有 0.08，是差异较稳定的一条。翻案腔是各家共同问题。
